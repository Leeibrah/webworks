...
<body>
    ...
    <ul data-role="listview" data-filter="true" rel="external">
		<?php
		include_once('includes/config.php');
		$sql = "select c.id, c.name, c.type, c.description, c.image, count(r.id) reportCount " . 
				"from art c left join art r on r.id = c.id " .
		...
		try {
			...
			while ($emp = $stmt->fetch(PDO::FETCH_OBJ)) {
				echo 
				 "<li><a href='artdetails.php?id=$emp->id'>" .
				 "<img src='../images/$emp->image'.'.png'/>" .
					"<h4>$emp->name</h4></a></li>";
